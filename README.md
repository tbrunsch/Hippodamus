...
When a task terminates, the method `Resource.release()` is called with the same value the task called `Resource.tryAcquire()` with. The `Resource` can then update internal information and also decide whether to resubmit postponed tasks.

When the coordinator is stopped, then `Resource.remove()` is called for every task with the `tryAgainRunnable` that is used to resubmit the task. The `Resource` can then clean up data that is related to the corresponding task if available.
